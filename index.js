...
// app.use(cors());
app.use(cors({
  origin:['https://assignment-11-library-cloud-web-app.netlify.app'],
  credentials:true
}))
...
async function run() {
  try {
    ...
    app.get("/allBooks",verifyFirebaseToken, async (req, res) => {
...
//GETTING ALL BOOKS ENDS HERE
    ...
    app.patch("/all-books/quantity/:id",verifyFirebaseToken, async (req, res) => {
      const id = req.params.id;
